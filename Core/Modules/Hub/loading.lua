...
local function GenerateName(Length)
    ...
    for i = 1, Length do
        Name[i] = Characters:sub(RNG:NextInteger(1, #Characters), RNG:NextInteger(1, #Characters))
    end

    return table.concat(Name)
end

Genv.SpecterXLoading = Genv.SpecterXLoading or {
...
local DEFAULTS = {
    ...
    MusicFadeTime = 1.2,
    GuiFadeTime   = 0.45,
    Title         = Hub:GetConfig("Title"),
    Subtitle      = Hub:GetConfig("SubTitle"),
}
...
function Loader:Run(RawConfig)
    ...
    if Genv.SpecterXLoading.Gui then
        Genv.SpecterXLoading.Gui:Destroy()
        Genv.SpecterXLoading.Gui = nil
    end
    ...
    local ScreenGui = New("ScreenGui", {
        Name = Genv.SpecterXLoading.Name,
        IgnoreGuiInset = true,
    ...
    Genv.SpecterXLoading.Gui = ScreenGui

    local Blur
    if Cfg.Blur then
        Blur = New("BlurEffect", {
            Name = "SpecterXLoadingBlur",
            Size = 0
        }, Lighting)

        TweenService:Create(
            Blur,
            TweenInfo.new(Cfg.BlurInTime, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
            {Size = Cfg.BlurSize}
        ):Play()
    end
    ...
    New("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Text = ("%s  •  %s  |  TODOS OS DIREITOS RESERVADOS"):format(Hub:GetConfig("Title"):upper(), os.date("%Y")),
        TextSize = 10,
    ...
    task.spawn(function()
        ...
        if Blur then
            local Tween = TweenService:Create(
                Blur,
                TweenInfo.new(Cfg.BlurFadeTime, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                {Size = 0}
            )
        ...
        if Music then
            local Tween = TweenService:Create(
                Music,
                TweenInfo.new(Cfg.MusicFadeTime, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                {Volume = 0}
            )
        ...
        local CloseTween = TweenService:Create(
            Window,
            TweenInfo.new(Cfg.GuiFadeTime, Enum.EasingStyle.Quad, Enum.EasingDirection.In),
            {BackgroundTransparency = 1}
        )
        ...
        if Genv.SpecterXLoading.Gui == ScreenGui then
            Genv.SpecterXLoading.Gui = nil
        end
