...
local function Encode(Data)
    ...
    if success then
        ...
        warn("[SpecterX] Falha ao codificar JSON:", result)
        return "{}"
    end
end
...
local function Decode(Data)
    ...
    if success then
        ...
        warn("[SpecterX] Falha ao decodificar JSON:", result)
        return {}
    end
end
...
local lib_config_path = {"Library", "config.json"}
local lib_config = LoadConfig(LibraryConfig, table.unpack(lib_config_path))
